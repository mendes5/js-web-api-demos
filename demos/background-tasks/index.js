...
const state = {
  ...
  scheduleTask: () => {
    ...
    const taskId = requestIdleCallback((idleDeadline) => {
      const runStartTime = performance.now();
      console.log(
        `Task ran after ${(runStartTime - scheduledTime).toFixed(2)}ms`,
        idleDeadline
      );
    });

    console.log("Task id: ", taskId, "  scheduled");
  },
};

const gui = new dat.GUI();

const MAX_POINT_COUNT = 200000;

gui.add(state, "maxPoints").min(100).max(MAX_POINT_COUNT).step(100);
gui.add(state, "scheduleTask");
...
const draw = (canvas, ctx, d) => {
  ...
  for (let [point, color] of points) {
    ...
    if (pointsRendered++ >= state.maxPoints) {
      break;
    }
