...
registerPaint(
  ...
  class {
    ...
    static get inputProperties() {
      return ["--gradient-color", "--pixel-size"];
    }

    paint(ctx, { width, height }, props) {
      ctx.fillStyle = props.get("--gradient-color") ?? "#f00";

      const pixelSize = Number(props.get("--pixel-size")) ?? 20;

      for (let x = 0; x <= width; x += pixelSize) {
        for (let y = 0; y <= height; y += pixelSize) {
          ctx.globalAlpha = (1 - x / width + Math.random() * 0.5) / 1.5;
...
registerPaint(
  ...
  class {
    ...
    static get inputProperties() {
      return ["--marker-color", "--thickness", "--size-factor"];
    }

    paint(ctx, { width, height }, props) {
      ctx.fillStyle = props.get("--marker-color") ?? "#f00";
      const sizeFactor = props.get("--size-factor") ?? 10;
      const thickness = props.get("--thickness") ?? 10;
